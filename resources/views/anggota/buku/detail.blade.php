    ...
    <div class="mx-auto max-w-5xl space-y-6">
        <div class="flex items-center justify-between gap-4">
            ...
            <a href="{{ route('anggota.buku') }}"
                class="inline-flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-50"><i
        ...
        <div class="grid gap-6 lg:grid-cols-[0.85fr_1.15fr]">
            ...
            <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="flex items-center gap-3">
                    <span
                        class="rounded-full {{ $buku->stok > 0 ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-rose-50 text-rose-700 border border-rose-200' }} px-3 py-1 text-xs font-semibold">{{ $buku->stok > 0 ? 'Tersedia' : 'Tidak Tersedia' }}</span>
                    <span
                        class="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">{{ $buku->kode_buku }}</span>
                </div>
                ...
                <div class="mt-6 flex gap-3">
                    @if ($buku->stok > 0)
                        <form action="{{ route('anggota.pinjam', $buku->id) }}" method="POST">
                            @csrf
                            <button type="submit" data-confirm data-confirm-title="Ajukan peminjaman?"
                                data-confirm-message="Pengajuan buku {{ $buku->judul }} akan dikirim ke petugas untuk diproses."
                                class="rounded-2xl bg-sky-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-sky-700">Pinjam
                                Buku</button>
                        </form>
                    @else
                        <button disabled
                            class="rounded-2xl bg-slate-200 px-5 py-3 text-sm font-semibold text-slate-500">Tidak
                            Tersedia</button>
                    @endif
                </div>
            </div>
