        ...
        <div class="overflow-x-auto">
            <table class="min-w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($data as $item)
                        <tr class="transition hover:bg-slate-50">
                            <td class="px-6 py-4">
                                ...
                                <p class="text-xs text-slate-500">ID Pinjam #{{ $item->id }}</p>
                            </td>
                            <td class="px-6 py-4 text-slate-600">
                                <p>{{ \Carbon\Carbon::parse($item->tanggal_pinjam)->format('d M Y') }}</p>
                                <p class="mt-1 font-medium {{ $item->status == 'terlambat' ? 'text-rose-600' : '' }}">
                                    {{ \Carbon\Carbon::parse($item->tanggal_wajib_kembali)->format('d M Y') }}</p>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                @if ($item->dendaData)
                                    <p class="font-semibold text-slate-900">Rp
                                        {{ number_format($item->dendaData->jumlah_denda, 0, ',', '.') }}</p>
                                    <p
                                        class="mt-1 text-xs font-semibold {{ $item->dendaData->status == 'belum_bayar' ? 'text-rose-600' : ($item->dendaData->status == 'menunggu_konfirmasi' ? 'text-amber-600' : 'text-emerald-600') }}">
                                        {{ $item->dendaData->status == 'belum_bayar' ? 'Belum dibayar' : ($item->dendaData->status == 'menunggu_konfirmasi' ? 'Menunggu konfirmasi petugas' : 'Sudah dibayar') }}
                                    </p>
                                @else
                                ...
                                @if ($item->dendaData && $item->dendaData->status == 'belum_bayar')
                                    <form action="{{ route('anggota.riwayat.bayar', $item->dendaData->id) }}"
                                        ...
                                        <button type="submit" data-confirm
                                            ...
                                            class="rounded-2xl bg-rose-600 px-4 py-2 text-xs font-semibold text-white transition hover:bg-rose-700">Saya
                                            Sudah Bayar</button>
                                    </form>
                                @elseif ($item->dendaData && $item->dendaData->status == 'menunggu_konfirmasi')
                                    <span
                                        class="inline-flex rounded-2xl border border-amber-200 bg-amber-50 px-4 py-2 text-xs font-semibold text-amber-700">Menunggu
                                        Konfirmasi</span>
                                @else
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center text-slate-400">Belum ada riwayat peminjaman.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
