        ...
        @if (session('success'))
            <div data-auto-dismiss
                ...
                <div class="flex items-center gap-3"><i
                        class="fas fa-circle-check"></i><span>{{ session('success') }}</span></div>
            </div>
        @endif
        @if (session('error'))
            <div data-auto-dismiss
            ...
            @forelse ($pinjam as $item)
                ...
                <article
                    ...
                    <div class="flex items-start justify-between gap-3">
                        <div>
                            <p class="text-lg font-bold text-slate-900">{{ $item->buku->judul }}</p>
                            <p class="mt-1 text-sm text-slate-500">Dipinjam
                                {{ \Carbon\Carbon::parse($item->tanggal_pinjam)->format('d M Y') }}</p>
                        </div>
                        <span
                            class="rounded-full {{ $terlambat > 0 ? 'border-rose-200 bg-rose-50 text-rose-700' : 'border-emerald-200 bg-emerald-50 text-emerald-700' }} border px-3 py-1 text-xs font-semibold">{{ $terlambat > 0 ? 'Terlambat ' . $terlambat . ' hari' : 'Masih aman' }}</span>
                    </div>
                    <div class="mt-5 grid gap-3 text-sm">
                        <div class="rounded-2xl bg-slate-50 p-4">
                            <p class="text-xs uppercase tracking-[0.16em] text-slate-400">Batas Kembali</p>
                            <p class="mt-2 font-semibold text-slate-900">{{ $wajib->format('d M Y') }}</p>
                        </div>
                        <div class="rounded-2xl bg-slate-50 p-4">
                            ...
                            <p class="mt-2 font-semibold {{ $denda > 0 ? 'text-rose-600' : 'text-slate-900' }}">
                                {{ $denda > 0 ? 'Rp ' . number_format($denda, 0, ',', '.') : 'Tidak ada' }}</p>
                        </div>
                    </div>
                    <form action="{{ route('anggota.pengembalian.proses') }}" method="POST" class="mt-5">
                        ...
                        <button type="submit" data-confirm data-confirm-title="Ajukan pengembalian?"
                            ...
                            class="w-full rounded-2xl bg-sky-600 px-4 py-3 text-sm font-semibold text-white transition hover:bg-sky-700">Kembalikan
                            Buku</button>
                    </form>
                </article>
            @empty
                <div
                    ...
                    Anda tidak memiliki buku aktif untuk dikembalikan.</div>
            @endforelse
        </section>
    </div>
@endsection
