...
<div class="p-6 max-w-2xl mx-auto">
    ...
    <form action="/petugas/buku/{{ $buku->id }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-5">
        @csrf
        @method('PUT')

        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1.5">Cover Buku</label>
            <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-200 border-dashed rounded-xl hover:border-blue-400 transition-colors cursor-pointer group relative">
                ...
                <img id="preview" src="{{ $buku->cover ? asset('storage/' . $buku->cover) : '' }}" class="{{ $buku->cover ? '' : 'hidden' }} absolute inset-0 w-full h-full object-cover rounded-xl shadow-sm">
                <button type="button" id="remove-preview" class="{{ $buku->cover ? '' : 'hidden' }} absolute top-2 right-2 bg-red-500 text-white p-1.5 rounded-full hover:bg-red-600 transition shadow-lg z-10" onclick="removeImage()">
        ...
        <div>
            ...
            <textarea name="deskripsi" rows="3"
                class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400"
                placeholder="Masukkan ringkasan atau keterangan buku">{{ old('deskripsi', $buku->deskripsi) }}</textarea>
        </div>
