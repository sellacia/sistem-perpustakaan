    ...
    <div class="mx-auto max-w-5xl space-y-6">
        <div class="flex items-center justify-between gap-4">
            <div>
                ...
                <p class="mt-2 text-sm text-slate-500">Informasi detail buku untuk kebutuhan monitoring kepala perpustakaan.
                </p>
            </div><a href="{{ route('kepala.buku.index') }}"
            ...
            <div class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
                <div class="aspect-[4/5] overflow-hidden bg-slate-100">
                    @if ($buku->cover)
                        ...
                        @else<div
                            class="flex h-full items-center justify-center text-slate-400">Tidak ada gambar</div>
                    @endif
                </div>
            </div>
            <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="flex items-center gap-3"><span
                        class="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">{{ $buku->kode_buku }}</span><span
                        class="rounded-full {{ $buku->stok > 0 ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-rose-50 text-rose-700 border border-rose-200' }} px-3 py-1 text-xs font-semibold">{{ $buku->status }}</span>
                </div>
