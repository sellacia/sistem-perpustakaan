...
<div id="action-modal" class="hidden fixed inset-0 z-[100] items-center justify-center bg-slate-950/55 p-4">
    <div class="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl">
        ...
        <div class="mt-6 flex justify-end gap-3">
            <button type="button" id="action-modal-cancel" class="rounded-2xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">
                Batal
            </button>
            <button type="button" id="action-modal-confirm" class="rounded-2xl bg-sky-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-sky-700">
                Ya, lanjutkan
            </button>
        </div>
    </div>
    ...
    (() => {
        ...
        document.addEventListener('click', (event) => {
            ...
            openActionModal(title, message, () => {
                if (form && trigger.tagName === 'BUTTON') {
                    form.submit();
                    return;
                }
