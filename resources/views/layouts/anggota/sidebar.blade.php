<div class="fixed left-0 top-0 z-50 flex h-screen w-72 flex-col overflow-hidden bg-[radial-gradient(circle_at_top,_rgba(56,189,248,0.28),_transparent_32%),linear-gradient(180deg,_#0f172a_0%,_#111827_48%,_#172554_100%)] text-white shadow-2xl">
    <div class="border-b border-white/10 px-6 py-6">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <h1 class="mt-1 text-lg font-bold">Panel Anggota</h1>
            </div>
        </div>
    </div>
    ...
    <ul class="flex-1 space-y-1 overflow-y-auto px-4 pb-4 text-sm">
        @php
            $menus = [
                ['label' => 'Dashboard', 'icon' => 'fa-house', 'route' => route('anggota.dashboard'), 'active' => request()->is('anggota/dashboard')],
                ['label' => 'Katalog Buku', 'icon' => 'fa-book', 'route' => route('anggota.buku'), 'active' => request()->is('anggota/buku*')],
                ['label' => 'Riwayat Peminjaman', 'icon' => 'fa-clock-rotate-left', 'route' => route('anggota.peminjaman'), 'active' => request()->is('anggota/peminjaman*')],
                ['label' => 'Pengembalian', 'icon' => 'fa-rotate-left', 'route' => route('anggota.pengembalian'), 'active' => request()->is('anggota/pengembalian*')],
                ['label' => 'Riwayat & Denda', 'icon' => 'fa-receipt', 'route' => route('anggota.riwayat'), 'active' => request()->is('anggota/riwayat*')],
            ];
        @endphp
    ...
    <div class="border-t border-white/10 p-4">
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit" data-confirm data-confirm-title="Logout sekarang?" data-confirm-message="Sesi anggota akan diakhiri dan Anda kembali ke halaman login." class="flex w-full items-center justify-center gap-3 rounded-2xl bg-white/10 px-4 py-3 text-sm font-semibold text-white transition hover:bg-white/15">
