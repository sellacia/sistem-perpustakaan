<div
    ...
    <div class="border-b border-white/10 px-6 py-6">
        <div class="flex items-center gap-3">
            <div
                class="flex h-12 w-12 items-center justify-center rounded-2xl bg-white/10 text-lg text-cyan-200 backdrop-blur">
    ...
    <ul class="flex-1 space-y-1 overflow-y-auto px-4 pb-4 text-sm">
        ...
        @foreach ($menus as $menu)
            <li>
                <a href="{{ $menu['route'] }}"
                    class="group flex items-center justify-between rounded-2xl px-4 py-3 transition {{ $menu['active'] ? 'bg-white text-slate-900 shadow-lg shadow-cyan-950/20' : 'text-slate-200 hover:bg-white/8 hover:text-white' }}">
                    <span class="flex items-center gap-3 font-medium">
                        <i
                            class="fas {{ $menu['icon'] }} {{ $menu['active'] ? 'text-sky-600' : 'text-cyan-200/80 group-hover:text-cyan-200' }}"></i>
                        {{ $menu['label'] }}
                    </span>
                    @if ($menu['active'])
