...
(() => {
    ...
    const closeActionModal = () => { actionModal.classList.add('hidden'); actionModal.classList.remove('flex'); pendingAction = null; };
    const openActionModal = (title, message, action) => { titleNode.textContent = title || 'Lanjutkan proses?'; messageNode.textContent = message || 'Pastikan aksi ini sudah sesuai.'; pendingAction = action; actionModal.classList.remove('hidden'); actionModal.classList.add('flex'); };
    confirmButton.addEventListener('click', () => { if (pendingAction) pendingAction(); closeActionModal(); });
    ...
    document.addEventListener('click', (event) => {
        ...
        openActionModal(title, message, () => {
            if (form && (trigger.tagName === 'BUTTON' || trigger.type === 'submit')) return form.submit();
            if (href) window.location.href = href;
        });
    });
    document.querySelectorAll('[data-auto-dismiss]').forEach((alert) => {
        setTimeout(() => { alert.classList.add('opacity-0', '-translate-y-2'); setTimeout(() => alert.remove(), 250); }, 3200);
    });
})();
</script>
</body>
