...
<div
    ...
    <ul class="flex-1 space-y-1 overflow-y-auto px-4 pb-4 text-sm">
        ...
        @php
            // Data menu khusus kepala
            $menus = [
                [
                    'label' => 'Dashboard',
                    'icon' => 'fa-house',
                    'route' => route('kepala.dashboard'),
                    'active' => request()->is('kepala/dashboard'),
                ],
                [
                    'label' => 'Katalog Buku',
                    'icon' => 'fa-book',
                    'route' => route('kepala.buku.index'),
                    'active' => request()->is('kepala/buku*'),
                ],
                [
                    'label' => 'Laporan',
                    'icon' => 'fa-chart-column',
                    'route' => route('kepala.laporan'),
                    'active' => request()->is('kepala/laporan*'),
                ],
                [
                    'label' => 'Data Petugas',
                    'icon' => 'fa-user-tie',
                    'route' => route('kepala.petugas.index'),
                    'active' => request()->is('kepala/petugas*'),
                ],
            ];
        @endphp
    ...
    <div class="border-t border-white/10 p-4">
        ...
        <form method="POST" action="{{ route('logout') }}">
            ...
            <button type="submit" data-confirm data-confirm-title="Logout sekarang?"
                data-confirm-message="Sesi kepala perpustakaan akan diakhiri dan Anda kembali ke halaman login."
